...
- **写真アップロード**: GPS情報付き写真の自動処理とS3アップロード
- **プレビューマーカー**: 写真選択時の位置プレビュー表示
- **インタラクティブマップ**: OpenStreetMapベースの日本全国マップ
- **モンスターボールマーカー**: ポケふた位置の視覚的表示
- **写真詳細表示**: フルサイズ画像と撮影情報の表示
- **写真削除機能**: 確認ダイアログ付きの安全な削除
- **スプラッシュスクリーン**: ランダム画像付きの起動画面
- **画像処理**: 720px幅への自動リサイズと圧縮
- **フォールバック機能**: S3失敗時のローカルストレージ保存
...
│   │   └── LocationPicker.tsx    # 手動位置選択コンポーネント
│   └── UI/
...
│   └── useDeletePhoto.tsx       # 写真削除機能フック
├── types/
│   └── photo.ts                 # 写真データ型定義
└── utils/
    ├── exifReader.ts            # EXIF データ読み取り
    ├── imageProcessor.ts        # 画像処理（リサイズ・圧縮）
    ├── localStorageUpload.ts    # ローカルストレージフォールバック
    ├── randomImage.ts           # ランダム画像選択
...
├── public/
│   ├── photos/
│   │   └── {year}/{month}/{uuid}.{ext}    # 処理済み画像（720px幅）
│   └── thumbnails/
│       └── {year}/{month}/{uuid}_thumb.jpg # サムネイル（将来的に追加）
└── local-fallback/
    └── localStorage で管理される写真データ（S3失敗時）
```
...
#### プロパティ12: プレビューマーカー表示の正確性
*任意の*GPS情報付き写真選択に対して、システムは即座にマップ上にプレビューマーカーを表示し、アップロード完了時にプレビューマーカーを削除する必要がある
**検証対象: 要件 1.2, 2.7**

#### プロパティ13: 写真削除の完全性
*任意の*写真削除操作に対して、システムはS3とDynamoDBから写真データを完全に削除し、マップからマーカーを即座に削除する必要がある
**検証対象: 要件 3.5, 3.6**

#### プロパティ14: 画像処理の一貫性
*任意の*アップロード画像に対して、720px幅への変換処理後もアスペクト比が保持され、ファイルサイズが適切に削減される必要がある
**検証対象: 要件 1.3, 1.4**

#### プロパティ15: フォールバック機能の信頼性
*任意の*S3アップロード失敗に対して、システムはローカルストレージに自動的にフォールバックし、データの整合性を保つ必要がある
**検証対象: 要件 1.7**
