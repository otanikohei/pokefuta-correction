...
2. 写真選択時にGPS情報が取得された場合、システムはマップ上にプレビューマーカーを表示する
3. 横 720px 以上のサイズがある JPEG 画像の場合は、横 720px (アスペクト比は元の画像を保持)に変換する
4. HEIC 画像の場合は、メタデータを保持したまま、横 720px (アスペクト比は元の画像を保持)に変換する
5. 写真にGPSメタデータが含まれない場合は、OpenStreetMapで座標をポイントできるウィザードを表示する
6. ユーザーが写真をアップロードしたとき、システムは画像ファイルをS3バケットに保存する
7. S3アップロードが失敗した場合、システムはローカルストレージにフォールバックして保存する
8. 写真アップロードする前に、システムは写真にGPSメタデータが含まれていることを検証する
9. 写真にGPS情報が含まれていない場合、システムはアップロードを拒否し、OpenStreetMapで座標をポイントするようウィザードを表示し、ユーザーにポイントを促す
10. 写真がアップロードされたら、システムはGPS座標とS3パス、撮影日時をAppSync GraphQL経由でDynamoDBに保存する
11. アップロード成功時、システムはマップを該当位置に自動フォーカスし、新しいマーカーをハイライト表示する
...
3. データベースから取得したGPS情報が存在するとき、システムは各写真の位置にモンスターボールアイコンのマーカーを表示する
4. マーカーを表示するとき、システムは32ピクセルのモンスターボールアイコンを使用する
5. 複数の写真が存在するとき、システムはすべてのマーカーをマップ上に同時に表示する
6. 新しくアップロードされた写真のマーカーは、5秒間ハイライト効果で強調表示される
7. 写真選択時にGPS情報が取得された場合、システムはプレビューマーカーを半透明で表示する
8. マップはピンが含まれる範囲をズームまたはパンする
9. システムはマーカーの可視性とパフォーマンスを維持する
10. 登録済み写真数をマップ上に表示する
...
3. 写真詳細を表示するとき、システムは撮影日時とアップロード日時を表示する
4. 写真詳細表示には写真を削除するボタンが含まれる
5. 写真削除時、システムは確認ダイアログを表示し、確認後にS3とDynamoDBから写真を削除する
6. 写真削除後、システムはマップからマーカーを即座に削除し、写真リストを更新する
7. 詳細表示が開いているとき、システムは閉じてマップに戻る方法を提供する
8. 写真詳細を読み込むとき、システムは読み込み状態を適切に処理する
...
**ユーザーストーリー:** ポケふた収集者として、使いやすく魅力的なインターフェースでアプリを使用したい。そうすることで、ポケふた収集がより楽しい体験になる。

#### 受入基準

1. アプリ起動時、システムはスプラッシュスクリーンを2秒間表示する
2. スプラッシュスクリーンには、セッション毎にランダムに選択されたポケふた画像を表示する
3. ヘッダーには、スプラッシュスクリーンと同じランダム画像を表示する
4. システムは読み込み中にエレガントなスピナーアニメーションを表示する
5. 新しくアップロードされたマーカーは、視覚的なハイライト効果で強調表示される
6. プレビューマーカーは、半透明でパルス効果を持つ青いスタイルで表示される